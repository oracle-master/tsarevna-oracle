...
=====================================
        ...
        Версия: автопробуждение + NFC-разблокировка

        Здесь:
        • состояния Царевны
        • пробуждение (автоматическое)
        • ожидание вопроса
        • поиск ответа
        • возвращение
        • NFC-слушатель
        • свечение зеркала и звёзды ✨
        • микро-касание для разблокировки NFC
...
function getOracleAnswer(){
    ...
    switch(mood){
        ...
        case 0:
            return {
                text: randomItem(positiveAnswers),
                type: "positive"
            };

        case 1:
            return {
                text: randomItem(neutralAnswers),
                type: "neutral"
            };

        default:
            return {
                text: randomItem(negativeAnswers),
                type: "negative"
            };
...
function showFinalAnswer(data) {
    ...
    // Убираем старые классы свечения
    if (mirror) {
        mirror.classList.remove("positive-glow", "neutral-glow", "negative-glow");
    }
    ...
    switch(data.type){
        ...
        case "negative":
            color = "#D870FF";
            glowClass = "negative-glow";
            break;

    }


    // Добавляем свечение на зеркало
    if (mirror && glowClass) {
        mirror.classList.add(glowClass);
    }
...
function wakePrincess() {
    ...
    setTimeout(() => {
        ...
        setText("О чём ты хочешь спросить?");
...
function startThinking() {
    ...
    if (mirror) {
        ...
        mirror.classList.add("spin");

    }


    // Во время вращения появляются очки
    setTimeout(() => {
...
/*
Возврат ко сну → потом снова пробуждение
*/
...
function returnToSleep() {
    ...
    setTimeout(() => {
        ...
        // Автоматическое пробуждение через 2 секунды после сна
        setTimeout(() => {
            wakePrincess();
        }, 2000);
...
- Автопробуждение
- NFC-слушатель
- Микро-касание для разблокировки NFC на проблемных телефонах
*/

window.addEventListener("load", () => {

    // Пробуждаем Царевну автоматически
    wakePrincess();
...
/*
Микро-касание — разблокировка NFC для телефонов,
где scan() не запускается без действия пользователя.

Никаких кнопок. Просто коснуться фона.
*/

document.addEventListener("click", (event) => {

    // Если NFC уже запущен — ничего не делаем
    if (isScanning) return;

    // Если это не касание фона (body), а что-то другое — пропускаем
    if (event.target !== document.body && event.target !== document.querySelector(".stars-layer")) return;

    // Пробуем запустить NFC повторно
    startNFCListening();
...
console.log("👑 Oracle 2.0 — автопробуждение + NFC активированы");
